    ...
    def from_json(cls, raw: str):
        ...
        for item in data:
            try:
                ...
                if not policy.client_id or not policy.license_id or not policy.product_id:
                    raise ValueError
                ...
                if policy.requests_per_minute < 1 or policy.daily_request_quota < 1:
                    raise ValueError
                if policy.token_sha256 in seen_hashes or policy.client_id in seen_clients:
                    raise ValueError
                seen_hashes.add(policy.token_sha256)
                seen_clients.add(policy.client_id)
